...
public class HlsWebServer : IAsyncDisposable
{
    ...
    private string GenerateStreamsPage()
    {
        ...
        sb.AppendLine("    .server-status { display: flex; align-items: center; justify-content: center; gap: 8px; margin-bottom: 16px; padding: 8px 16px; border-radius: 20px; font-size: 13px; font-weight: 500; width: fit-content; margin-left: auto; margin-right: auto; }");
        sb.AppendLine("    .server-status.online { background: linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%); color: #2e7d32; }");
        sb.AppendLine("    .server-status.offline { background: linear-gradient(135deg, #ffebee 0%, #ffcdd2 100%); color: #c62828; }");
        sb.AppendLine("    .server-status.checking { background: linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%); color: #e65100; }");
        sb.AppendLine("    .status-dot { width: 10px; height: 10px; border-radius: 50%; }");
        sb.AppendLine("    .server-status.online .status-dot { background: #4caf50; animation: pulse-green 2s infinite; }");
        sb.AppendLine("    .server-status.offline .status-dot { background: #f44336; }");
        sb.AppendLine("    .server-status.checking .status-dot { background: #ff9800; animation: pulse-orange 1s infinite; }");
        sb.AppendLine("    @keyframes pulse-green { 0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(76,175,80,0.4); } 50% { opacity: 0.8; box-shadow: 0 0 0 6px rgba(76,175,80,0); } }");
        sb.AppendLine("    @keyframes pulse-orange { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }");
        ...
        sb.AppendLine("    .stream-header { display: flex; align-items: center; gap: 16px; flex-wrap: wrap; }");
        ...
        sb.AppendLine("    .stream-name-row { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }");
        sb.AppendLine("    .stream-name { font-size: 18px; font-weight: 600; color: #2c3e50; }");
        sb.AppendLine("    .btn-info-toggle { background: none; border: 1px solid #ccc; border-radius: 12px; padding: 2px 10px; font-size: 11px; color: #666; cursor: pointer; transition: all 0.2s; }");
        sb.AppendLine("    .btn-info-toggle:hover { background: #f0f0f0; border-color: #999; }");
        ...
        sb.AppendLine("    .player-controls { display: flex; align-items: center; gap: 12px; padding: 14px 16px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 10px; border: 1px solid #dee2e6; flex-wrap: wrap; }");
        ...
        sb.AppendLine("</head>");
        sb.AppendLine("<body>");
        sb.AppendLine("  <div id=\"server-status\" class=\"server-status checking\">");
        sb.AppendLine("    <span class=\"status-dot\"></span>");
        sb.AppendLine("    <span class=\"status-text\">Checking server...</span>");
        sb.AppendLine("  </div>");
        ...
        foreach (var stream in _streams.Where(s => s.IsEnabled))
        {
            ...
            sb.AppendLine($"          <div class=\"stream-name-row\">");
            sb.AppendLine($"            <span class=\"stream-name\">{System.Net.WebUtility.HtmlEncode(stream.Name)}</span>");
            sb.AppendLine($"            <button class=\"btn-info-toggle\" onclick=\"toggleStreamInfo({streamIndex})\">Stream info</button>");
            sb.AppendLine($"          </div>");
            sb.AppendLine($"          <div class=\"stream-details\" id=\"stream-details-{streamIndex}\">");
        ...
        sb.AppendLine("    }");
        sb.AppendLine("    ");
        sb.AppendLine("    function toggleStreamInfo(index) {");
        sb.AppendLine("      const details = document.getElementById(`stream-details-${index}`);");
        sb.AppendLine("      const btn = details.parentElement.querySelector('.btn-info-toggle');");
        sb.AppendLine("      if (details.classList.contains('expanded')) {");
        sb.AppendLine("        details.classList.remove('expanded');");
        sb.AppendLine("        btn.classList.remove('expanded');");
        sb.AppendLine("      } else {");
        sb.AppendLine("        details.classList.add('expanded');");
        sb.AppendLine("        btn.classList.add('expanded');");
        sb.AppendLine("      }");
        sb.AppendLine("    }");
        sb.AppendLine("    ");
        sb.AppendLine("    // Server status check");
        sb.AppendLine("    function checkServerStatus() {");
        sb.AppendLine("      const statusEl = document.getElementById('server-status');");
        sb.AppendLine("      const textEl = statusEl.querySelector('.status-text');");
        sb.AppendLine("      ");
        sb.AppendLine("      fetch('/health', { method: 'GET', cache: 'no-store' })");
        sb.AppendLine("        .then(response => {");
        sb.AppendLine("          if (response.ok) {");
        sb.AppendLine("            statusEl.className = 'server-status online';");
        sb.AppendLine("            textEl.textContent = 'Server online';");
        sb.AppendLine("          } else {");
        sb.AppendLine("            statusEl.className = 'server-status offline';");
        sb.AppendLine("            textEl.textContent = 'Server offline';");
        sb.AppendLine("          }");
        sb.AppendLine("        })");
        sb.AppendLine("        .catch(() => {");
        sb.AppendLine("          statusEl.className = 'server-status offline';");
        ...
        sb.AppendLine("    }");
        sb.AppendLine("    ");
        sb.AppendLine("    // Check server status on load and every 5 seconds");
        sb.AppendLine("    checkServerStatus();");
        sb.AppendLine("    setInterval(checkServerStatus, 5000);");
        sb.AppendLine("  </script>");
        sb.AppendLine("</body>");
        sb.AppendLine("</html>");
